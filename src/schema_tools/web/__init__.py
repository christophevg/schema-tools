...
from schema_tools import peppol, xml
...
app = Flask("schema-tools", template_folder=TEMPLATES, static_folder=STATIC)
...
def protected(func):
  def wrapper():
    try:
      ...
      logger.error(
        f"[bold red]Whoops:[/bold red] {ex} ({ex.__class__.__module__}.{ex.__class__.__qualname__})",
        extra={"markup": True},
      )
...
@app.route("/")
def home():
  return render_template("index.html", markdown=markdown_template, xml=xml_example)
...
def view():
  ...
  namespaces = {uri: None for key, uri in xml[root].items() if key.startswith("@xmlns")}
  ...
  template = SandboxedEnvironment(loader=BaseLoader(), undefined=SilentUndefined).from_string(
    meta_template
  )
  html = markdown(
    template.render(xml=xml, ensure_list=ensure_list), extensions=["tables", "attr_list"]
  )
  return render_template("render.html", body=html, debug=json.dumps(xml, indent=2))
