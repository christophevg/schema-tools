...
<schema xmlns="http://purl.oclc.org/dsdl/schematron" xmlns:u="utils" schemaVersion="iso"
  ...
  <!-- Functions -->
  <function xmlns="http://www.w3.org/1999/XSL/Transform" name="fn:gln" as="xs:boolean">
    <param name="val" />
  ...
  <function xmlns="http://www.w3.org/1999/XSL/Transform" name="fn:slack" as="xs:boolean">
    <param name="exp" as="xs:decimal" />
    <param name="val" as="xs:decimal" />
    <param name="slack" as="xs:decimal" />
    <value-of select="xs:decimal($exp + $slack) &gt;= $val and xs:decimal($exp - $slack) &lt;= $val" />
  </function>
  <function xmlns="http://www.w3.org/1999/XSL/Transform" name="fn:mod11" as="xs:boolean">
    <param name="val" />
    <variable name="length" select="string-length($val) - 1" />
    <variable name="digits"
      select="reverse(for $i in string-to-codepoints(substring($val, 0, $length + 1)) return $i - 48)" />
  ...
  <function xmlns="http://www.w3.org/1999/XSL/Transform" name="fn:mod97-0208" as="xs:boolean">
    <param name="val" />
  ...
  <function name="fn:checkCodiceIPA" as="xs:boolean" xmlns="http://www.w3.org/1999/XSL/Transform">
  ...
  <function name="fn:checkCF" as="xs:boolean" xmlns="http://www.w3.org/1999/XSL/Transform">
    <param name="arg" as="xs:string?" />
    <sequence
      select="
  ...
  <function name="fn:checkCF16" as="xs:boolean" xmlns="http://www.w3.org/1999/XSL/Transform">
    <param name="arg" as="xs:string?" />
  ...
  <function name="fn:checkPIVAseIT" as="xs:boolean" xmlns="http://www.w3.org/1999/XSL/Transform">
    <param name="arg" as="xs:string" />
    ...
    <sequence
      select="
			...
			then
			(
				if ( ( string-length($codice) = 11 ) and ( if (fn:checkPIVA($codice)!=0) then false() else true() ))
				then
				(
					true()
				)
  ...
  <function name="fn:checkPIVA" as="xs:integer" xmlns="http://www.w3.org/1999/XSL/Transform">
    <param name="arg" as="xs:string?" />
    <sequence
      select="
				if (not($arg castable as xs:integer))
					then 1
					else ( fn:addPIVA($arg,xs:integer(0)) mod 10 )" />
  </function>
  <function name="fn:addPIVA" as="xs:integer" xmlns="http://www.w3.org/1999/XSL/Transform">
    <param name="arg" as="xs:string" />
    ...
    <sequence
      select="if ($tappo = 0) then $mapper else ( xs:integer($mapper) + fn:addPIVA(substring(xs:string($arg),2), (if($pari=0) then 1 else 0) ) )" />
  </function>
  <function xmlns="http://www.w3.org/1999/XSL/Transform" name="fn:abn" as="xs:boolean">
    <param name="val" />
    <value-of
      select="(
...
" />
  ...
  <function xmlns="http://www.w3.org/1999/XSL/Transform" name="fn:TinVerification" as="xs:boolean">
  ...
  <function xmlns="http://www.w3.org/1999/XSL/Transform" name="fn:checkSEOrgnr" as="xs:boolean">
  ...
  <pattern>
    ...
    <rule
      ...
      <assert id="PEPPOL-EN16931-R040"
        test="
          not(cbc:MultiplierFactorNumeric and cbc:BaseAmount) or fn:slack(if (cbc:Amount) then
            cbc:Amount
          else
    ...
    <rule context="//cac:InvoiceLine | //cac:CreditNoteLine">
      ...
      <assert id="PEPPOL-EN16931-R120"
        test="fn:slack($lineExtensionAmount, ($quantity * ($priceAmount div $baseQuantity)) + $chargesTotal - $allowancesTotal, 0.02)"
    ...
    <rule
      ...
      <assert id="PEPPOL-COMMON-R040"
        test="matches(normalize-space(), '^[0-9]+$') and fn:gln(normalize-space())" flag="fatal">GLN must have a valid format according to GS1 rules.</assert>
    </rule>
    <rule
      context="//cbc:EndpointID[@schemeID = '0192'] | //cac:PartyIdentification/cbc:ID[@schemeID = '0192'] | //cbc:CompanyID[@schemeID = '0192']">
      <assert id="PEPPOL-COMMON-R041"
        test="matches(normalize-space(), '^[0-9]{9}$') and fn:mod11(normalize-space())" flag="fatal">Norwegian organization number MUST be stated in the correct format.</assert>
    </rule>
    <rule
    ...
    <rule
      ...
      <assert id="PEPPOL-COMMON-R043"
        test="matches(normalize-space(), '^[0-9]{10}$') and fn:mod97-0208(normalize-space())"
    ...
    <rule
      ...
      <assert id="PEPPOL-COMMON-R044" test="fn:checkCodiceIPA(normalize-space())" flag="warning">IPA Code (Codice Univoco Unità Organizzativa) must be stated in the correct format</assert>
    </rule>
    <rule
      context="//cbc:EndpointID[@schemeID = '0210'] | //cac:PartyIdentification/cbc:ID[@schemeID = '0210'] | //cbc:CompanyID[@schemeID = '0210']">
      <assert id="PEPPOL-COMMON-R045" test="fn:checkCF(normalize-space())" flag="warning">Tax Code (Codice Fiscale) must be stated in the correct format</assert>
    </rule>
    <rule context="//cbc:EndpointID[@schemeID = '9907']">
      <assert id="PEPPOL-COMMON-R046" test="fn:checkCF(normalize-space())" flag="warning">Tax Code (Codice Fiscale) must be stated in the correct format</assert>
    </rule>
    <rule
    ...
    <rule context="//cac:AccountingSupplierParty/cac:Party[$supplierCountry = 'NO']">
      ...
      <assert id="NO-R-001"
        test="cac:PartyTaxScheme[normalize-space(cac:TaxScheme/cbc:ID) = 'VAT']/substring(cbc:CompanyID, 1, 2)='NO' and matches(cac:PartyTaxScheme[normalize-space(cac:TaxScheme/cbc:ID) = 'VAT']/substring(cbc:CompanyID,3), '^[0-9]{9}MVA$')
          and fn:mod11(substring(cac:PartyTaxScheme[normalize-space(cac:TaxScheme/cbc:ID) = 'VAT']/cbc:CompanyID, 3, 9)) or not(cac:PartyTaxScheme[normalize-space(cac:TaxScheme/cbc:ID) = 'VAT']/substring(cbc:CompanyID, 1, 2)='NO')"
  ...
  <pattern>
    ...
    <rule
      ...
      <assert id="SE-R-013" test="fn:checkSEOrgnr(normalize-space(cbc:CompanyID))" flag="fatal">The last digit of a Swedish organization number must be valid according to the Luhn algorithm.</assert>
    </rule>
    <rule
    ...
    <rule
      ...
      <assert id="GR-R-001-2"
        test="string-length(normalize-space($IdSegments[1])) = 9
			                              and fn:TinVerification($IdSegments[1])
    ...
    <rule context="//cac:AccountingSupplierParty[$isGreekSender]/cac:Party">
      ...
      <assert id="GR-S-011"
        test="count(cac:PartyTaxScheme[normalize-space(cac:TaxScheme/cbc:ID) = 'VAT']/cbc:CompanyID)=1 and
				                        ...
				                        fn:TinVerification(substring(cac:PartyTaxScheme[normalize-space(cac:TaxScheme/cbc:ID) = 'VAT']/cbc:CompanyID,3))"
