...
from schema_tools.nodes import ValueNode, ListNode, ObjectNode

def load(path):
  return yaml.load(path, Loader=YamlSchemaLoader)
...
def scalar_constructor(type=None):
  def constructor(loader, node):
    ...
    return ValueNode(value, node.start_mark.line+1, node.start_mark.column)      
  return constructor
...
def list_constructor(loader, node):
  ...
  return ListNode(items, node.start_mark.line, node.start_mark.column)
...
def dict_constructor(loader, node):
  ...
  return ObjectNode(mapping, node.start_mark.line, node.start_mark.column)
